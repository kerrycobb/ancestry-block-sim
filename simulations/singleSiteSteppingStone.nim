...
proc mating(source: Deme, recipient: var Deme, recombination, selection: float): bool = 
  ...
  if hap1.data.allele == hap2.data.allele: 
    if rand(1.0) < recombination:
      recipient.add(recombine(hap1, hap2, rand(1..hap1.len-1)))
    else:
      recipient.add(hap1)
    result = true
  else:
...
var 
  ...
  sitePos = 50_000
  recombRate = 1e-8 
  recombination = recombRate * (length-1).float #-1 because we don't want recombination at 0 
  selection = rand(1.0) # Probability that a heterozygote will be inherited
  outfile = "singleSiteSteppingStone-output.txt"
